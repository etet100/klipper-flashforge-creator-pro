...
sailfish_lcd_begin(struct sailfish_lcd *h)
{
  ...
  sailfish_lcd_write_serial(h, 0b00000000);

  // this is according to the hitachi HD44780 datasheet
  // figure 24, pg 46

  // we start in 8bit mode, try to set 4 bit mode
  sailfish_lcd_write_4bits(h, 0x03, false);
...
{
    struct sailfish_lcd *h = oid_alloc(args[0], command_config_sailfish_lcd, sizeof(*h));
    ...
    sailfish_lcd_begin(h);
}
DECL_COMMAND(command_config_sailfish_lcd,
             "config_sailfish_lcd oid=%c strobe_pin=%u data_pin=%u"
             " clk_pin=%u delay_ticks=%u");

void
command_sailfish_lcd_send_cmds(uint32_t *args)
{
    struct sailfish_lcd *h = oid_lookup(args[0], command_config_sailfish_lcd);
    ...
    while (len--) {
        ...
        while (timer_read_time() - last_cmd_time < cmd_wait_ticks)
            irq_poll();
        sailfish_lcd_send(h, b, false);
        last_cmd_time = timer_read_time();
    }
}
DECL_COMMAND(command_sailfish_lcd_send_cmds, "sailfish_lcd_send_cmds oid=%c cmds=%*s");

void
command_sailfish_lcd_send_data(uint32_t *args)
{
    struct sailfish_lcd *h = oid_lookup(args[0], command_config_sailfish_lcd);
    uint32_t last_cmd_time=h->last_cmd_time, cmd_wait_ticks=h->cmd_wait_ticks;
    uint8_t len = args[1], *data = (void*)(size_t)args[2];
    while (len--) {
        ...
        while (timer_read_time() - last_cmd_time < cmd_wait_ticks)
            irq_poll();
        sailfish_lcd_send(h, b, true);
        last_cmd_time = timer_read_time();
    }
}
DECL_COMMAND(command_sailfish_lcd_send_data, "sailfish_lcd_send_data oid=%c data=%*s");
